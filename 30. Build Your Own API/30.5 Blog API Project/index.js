...
app.get("/posts", (req, res) => {
  ...
  //   res.render("index.ejs", res.json(post));
  // }
...
app.get("/posts/:id", (req, res) => {
  let foundPost = posts.find((post) => post.id === parseInt(req.params.id));
  if (!foundPost)
...
app.delete("/posts/:id", (req, res) => {
  let searchIndex = posts.findIndex(
    (post) => post.id === parseInt(req.params.id)
  );
  if (searchIndex > -1) {
    posts.splice(searchIndex, 1);
    res.sendStatus(200).json({ message: "Post is been deleted" });
  } else {
    res
      .status(404)
